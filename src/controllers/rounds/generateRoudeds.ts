type Round = Array<{ casa: string; fora: string }>;
type Rounds = Record<string, Round>;

export function createRoundByUsers(players: string[]): Rounds {
  ...
  const rounds: { ida: Round[]; volta: Round[] } = { ida: [], volta: [] }; // Objeto para armazenar as rodadas de ida e volta
